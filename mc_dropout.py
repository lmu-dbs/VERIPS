...
# original from https://github.com/BlackHC/BatchBALD
# by Andreas Kirsch and Joost van Amersfoort and Yarin Gal
# for BatchBALD: Efficient and Diverse Batch Acquisition for Deep Bayesian Active Learning, 2019
...
class _MCDropout(Module):
    ...
    def forward(self, input: torch.Tensor):
        ...
        #print("forward_input " + str(input.shape))
        k = BayesianModule.k
        #k = 10
        k = input.shape[0]
        #print("k " + str(k))
        if self.training:
            ...
            self.mask = self._create_mask(input, k)
            mask = self.mask
        #print("mc_input " + str(input.shape))
        #print("k " + str(k))

        mc_input = BayesianModule.unflatten_tensor(input, k)

        #print("mc_input unflattened " + str(mc_input.shape))
        mc_output = mc_input.masked_fill(mask, 0) / (1 - self.p)

        #print("mc_output " + str(mc_output.shape))

        #print("output " + str(BayesianModule.flatten_tensor(mc_output).shape))
